...
cBall::cBall(ros::NodeHandle &nh, std::string& name, double& angle)
    : nh_(nh),name_(name),angle_(angle)
{
    ROS_INFO("Initialize Turtle Ball");


    turtlesim::Spawn spawn;
    spawn.request.name = name_;
    spawn.request.x = 2.0;
    spawn.request.y = 2.0;
    ...
    pose_sub_ = nh_.subscribe<turtlesim::Pose>("/"+name_+"/pose", 1, &cBall::poseCallback, this);
    ...
    cmd_vel_pub_ = nh_.advertise<geometry_msgs::Twist>("/"+name_+"/cmd_vel", 1);
    teleport_abs_client_ = nh_.serviceClient<turtlesim::TeleportAbsolute>("/"+name_+"/teleport_absolute");
...
void cBall::penOff(bool off)
{
    ...
    ros::service::call("/"+name_+"/set_pen", set_pen);
}
...
int main(int argc, char** argv)
 {
    ...
    std::vector<double> section_angle;
    std::vector<cBall> section_ball;
    std::string name;

    for (int i = 0 ; i < num; i++)
    {
        name = "ball" + std::to_string(i);
        section_name.push_back(name);
        section_angle.push_back(double(i));

    }
    cBall ball0(nh,section_name[0],section_angle[0]);
    cBall ball1(nh,section_name[1],section_angle[1]);
    cBall ball2(nh,section_name[2],section_angle[2]);
    cBall ball3(nh,section_name[3],section_angle[3]);
    cBall ball4(nh,section_name[4],section_angle[4]);
